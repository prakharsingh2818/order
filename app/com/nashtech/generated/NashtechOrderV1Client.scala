...
package com.nashtech.order.v1 {
  ...
  class Client(
    ...
    private[this] val logger = play.api.Logger("com.nashtech.Client")

    logger.info(s"Initializing com.nashtech.Client for url $baseUrl")
    ...
    object Orders extends Orders {
      override def getByNumber(
        ...
        _executeRequest("GET", s"/${play.utils.UriEncoding.encodePathSegment(merchantId, "UTF-8")}/orders/${play.utils.UriEncoding.encodePathSegment(number, "UTF-8")}", requestHeaders = requestHeaders).map {
          case r if r.status == 200 => _root_.com.nashtech.order.v1.Client.parseJson("com.nashtech.Order", r, _.validate[com.nashtech.order.v1.models.Order])
